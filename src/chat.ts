...
import { Usuario } from './types/usuario.type';
 ...
 export class Chat {
   ...
   adicionarConexao( conexao: WebSocket) {
    const user = new Usuario('id-desconhecido', 'desconhecido', new Date());
    ...
    conexao.on('message', async (processo) => {
      try {
        ...
        if (body instanceof MsgRecebidaNovoUsuario) {
          // Atualiza o nome do usuário na lista de conexões com todos os campos necessários
          this.conexoes.set(conexao, {
            id: `id-${Date.now()}`, // Gera um ID único
            nome: body.nomeUsuario,
            data_criacao: new Date(), // Define a data atual como a data de criação
          });
  ...
  const msg = new MsgSaidaNovoUsuario(body.nomeUsuario);
